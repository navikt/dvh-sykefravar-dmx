{{ config(
    materialized='table',
    post_hook= ["grant READ ON dvh_syfo.fak_dialogmote_hendelse_tidspunkt to DVH_SYK_DBT"]
)}}


WITH hendelser AS (
  SELECT * FROM {{ref("mk_dialogmote__pivotert_hendelse_tidspunkt")}}
)
...
,motebehov AS (
  SELECT * FROM {{ ref('mk_motebehov__prepp_hendelse_tidspunkt') }}
)
...
,final AS (
  SELECT
    ...
    ,NVL(TO_NUMBER(TO_CHAR(motebehov.behov_meldt_dato, 'YYYYMMDD')), -1) AS fk_dim_tid__behov_meldt
    ,NVL(TO_NUMBER(TO_CHAR(hendelser.tilfelle_startdato, 'YYYYMMDD')), -1) AS fk_dim_tid__tilfelle_startdato
    ,NVL(TO_NUMBER(TO_CHAR(dialogmote2_avholdt_dato, 'YYYYMMDD')), -1) AS fk_dim_tid__dm2_avholdt_dato
    ,NVL(TO_NUMBER(TO_CHAR(dialogmote3_avholdt_dato, 'YYYYMMDD')), -1) AS fk_dim_tid__dm3_avholdt_dato
    ,NVL(TO_NUMBER(TO_CHAR(dialogmote4_avholdt_dato, 'YYYYMMDD')), -1) AS fk_dim_tid__dm4_avholdt_dato
    ,NVL(TO_NUMBER(TO_CHAR(dialogmote5_avholdt_dato, 'YYYYMMDD')), -1) AS fk_dim_tid__dm5_avholdt_dato
    ,NVL(TO_NUMBER(TO_CHAR(dialogmote6_avholdt_dato, 'YYYYMMDD')), -1) AS fk_dim_tid__dm6_avholdt_dato
    ,NVL(TO_NUMBER(TO_CHAR(dialogmote7_avholdt_dato, 'YYYYMMDD')), -1) AS fk_dim_tid__dm7_avholdt_dato
    ,NVL(TO_NUMBER(TO_CHAR(unntak, 'YYYYMMDD')), -1) AS fk_dim_tid__unntak_dato
