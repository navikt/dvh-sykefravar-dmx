  ...
  from (
        select
          ...
          row_number() over(
            ...
            order by (uforhet.vurderingsdato) - (sf_oppfolging.endret_kilde_dato)
          ) rn
        from uforhet
    left join sf_oppfolging on uforhet.fk_person1 = sf_oppfolging.fk_person1
      and uforhet.vurderingsdato >= sf_oppfolging.endret_kilde_dato --ok?
    )
  where rn = 1
),
...
join_alle as (
  ...
  left join sett_39_uker_flagg b
    on a.fk_person1 = b.fk_person1 and a.vurderingsdato = b.vurderingsdato and a.vurderingstype = b.vurderingstype
  left join tildelt_enhet_tilfelle_startdato c
    on a.fk_person1 = c.fk_person1 and a.vurderingsdato = c.vurderingsdato and a.vurderingstype = c.vurderingstype
),

{{ add_dimensjoner(ref('mk_arbeidsuforhet__prepp')) }}



--select * from dimensjoner
