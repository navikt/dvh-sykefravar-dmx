...
with person_oversikt_status as (
  select
    ...
    kildesystem,
    lastet_dato,
    oppdatert_dato as oppdatert_dato_old, --må fjernes fra kildeinnlasting i DAG syfo_kandidater_enhet
    dbt_scd_id,
    dbt_updated_at,
    dbt_valid_from,
    dbt_valid_to
  from {{ source('modia', 'fk_syfo_person_oversikt_status__snapshot')}}
--from {{ ref('fk_modia__person_oversikt_status_scd_test')}} -- bruk ved test
),
...
sett_gyldig_kolonner as (
    select
        ...
        trunc(sysdate) as oppdatert_dato --endre til systdate-1 ved test
