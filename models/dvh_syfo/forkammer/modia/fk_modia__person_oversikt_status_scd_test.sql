...
with person_oversikt_status_snapshot as (
  ...
  where trunc(lastet_dato) != to_date('15.05.2024', 'dd.mm.yyyy') -- for stopp lasting av gårsdagens data eller lenger tilbake for testing
),
...
rader_til_operasjon as (
    select * from person_oversikt_status_snapshot
    where fk_person1 in (select fk_person1 from nyeste_rader) 
),
...
sett_gyldig_kolonner as (
    select
        ...
        trunc(sysdate-1) as oppdatert_dato_dbt --endre oppdatert dato ut ifra når testkjøringene skal være
