  ...
  SELECT
      ...
      CAST(TO_TIMESTAMP_TZ(JSON_VALUE(oppfolgingstilfelle.KAFKA_MESSAGE, '$.stoppunktAt'), 'YYYY-MM-DD HH24:MI:SS.FF:TZH:TZM') at TIME ZONE 'CET' as timestamp) AS stoppunktAt,
       CAST(TO_TIMESTAMP_TZ(JSON_VALUE(oppfolgingstilfelle.KAFKA_MESSAGE,'$.createdAt'), 'yyyy-mm-dd"T"hh24:mi:ss.fftzh:tzm"Z"') at TIME ZONE 'CET' as timestamp) as createdAt,
        JSON_VALUE(oppfolgingstilfelle.KAFKA_MESSAGE, '$.antallSykedager') as antallSykedager,
        json_value(oppfolgingstilfelle.kafka_message,'$.updatedBy') as updatedBy,
        CAST(TO_TIMESTAMP_TZ(JSON_VALUE(oppfolgingstilfelle.kafka_message,'$.start'), 'yyyy-mm-dd"T"hh24:mi:ss.fftzh:tzm"Z"') at TIME ZONE 'CET' as timestamp) AS start_dato,
        CAST(TO_TIMESTAMP_TZ(JSON_VALUE(oppfolgingstilfelle.kafka_message,'$.stop'), 'yyyy-mm-dd"T"hh24:mi:ss.fftzh:tzm"Z"') at TIME ZONE 'CET' as timestamp) AS stopp_dato,
    kafka_topic,
    kafka_partisjon,
    kafka_offset,
    kafka_mottatt_dato,
