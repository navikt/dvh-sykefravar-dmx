...
, join_fk_person_sm AS (
    SELECT
        motebehov.*,
        dvh_person_ident.fk_person1 as fk_person1_sm  -- den sykemeldte
        from motebehov
    LEFT JOIN dvh_person_ident
    ON
      motebehov.sm_fnr = dvh_person_ident.off_id
      AND dvh_person_ident.gyldig_til_dato = TO_DATE('9999-12-31', 'YYYY-MM-DD') -- Kanskje byttes det til opprettet dato, da vet en når denne meldinge ble mottatt
)
, join_fk_person1_2 as (
    select
    join_fk_person_sm.*,
    dvh_person_ident.fk_person1 as fk_person1_behov  -- den som har meldt behovet
